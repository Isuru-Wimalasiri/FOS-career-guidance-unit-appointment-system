...
const StudentProfile = (props) => {
  ...
  const handleSideClick = (index) => {
    index === 0 ? setIsHistory(true) : setIsHistory(false);
  };
  ...
  const drawer = (
    <div>
      ...
      <List>
        {['History', 'Edit Details'].map((text, index) => (
          <ListItem
            key={text}
            disablePadding
            style={
              (isHistory && index === 0) || (!isHistory && index === 1)
          ...
          >
            <ListItemButton onClick={() => handleSideClick(index)}>
              <ListItemIcon>
                {index % 2 === 0 ? <HistoryIcon /> : <EditIcon />}
              </ListItemIcon>
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        >
          ...
          <div className="profileSection">
            {isHistory ? <div>Print</div> : <EditProfile />}
          </div>
        </Box>
      </Box>
    </div>
